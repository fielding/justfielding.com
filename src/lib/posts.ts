...
function buildMeta(path: string): PostMeta {
	...
	return {
		...
		// build step. The content hash lives in the filename because scrapers
		// cache images by URL and LinkedIn's ignores query strings entirely.
		image:
			parsed.metadata?.image ??
			(ogHashes[slug] ? `/img/og/${slug}.${ogHashes[slug]}.png` : `/img/og/${slug}.png`)
	};
}
